...
Start at the DOS Header to find e_lfanew, the file offset to the NT Headers (0xF8 in the example).

The File Header follows the PE Signature in the NT Headers. Since the PE Signature is a DWORD (4 bytes), the File Header starts at the file offset 0xFC.

Calculate the size of the File Header. It consists of 4 WORDs and 3 DWORDs:
...
Add the size of the Optional Header (0xE0 in the example) to the file offset of the Optional Header to get the start of the Section Table:
...
The loader looks at the Hint value in the IMAGE_IMPORT_BY_NAME structure for a given imported function.

It goes to the Export Name Pointer Table of the DLL specified by the import and checks the function name at the hint index.

If the name at the hint index matches the Name in the IMAGE_IMPORT_BY_NAME, the loader uses this information to resolve the address of the function quickly.

If the hint is incorrect, the loader searches through the Export Name Pointer Table to find the correct entry that matches the Name.

Once the correct name is found, the loader retrieves the corresponding ordinal from the Export Ordinal Table.
...
1- By Name: The executable contains an import directory with an entry for each imported function or variable. This entry includes a hint and the name of the import, as described by the IMAGE_IMPORT_BY_NAME structure.

2- By Ordinal: Instead of using a name, the executable can import functions using their ordinal numbers. This method does not require the importing executable to know the names of the functions, just their ordinal positions within the export table of the DLL.

Calculation Examples
The file offset of the end of the .text section is calculated as:
Offset of ‘.text‘+Size of ‘.text‘=0x400+0xB200=0xB600Offset of ‘.text‘+Size of ‘.text‘=0x400+0xB200=0xB600

The Virtual Address of the .text section is calculated by adding its RVA to the base address:
...
image
image
We use a commmand after ret from P/ P/ R/ to assemmble a jmp instruction.
We choose the address we want to jump to jmp 0xaddress.
Press Enter to assemble
We can see in the opcode that the JMP 0xaddress translated to eb08 which means Jump 08 bytes which is gonna jump to the address we provided (later our shellcode).
